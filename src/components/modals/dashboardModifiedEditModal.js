...
export class DashboardModifiedEditModal extends Component {
    ...
    render() {
        return (
            <div className="text-dark">
                <button id={this.props.id+'ModalOpen'} type="button" className="d-none" data-toggle="modal" data-target={'#'+this.props.id+'ModifiedEditModal'}>Open</button>

                <div id={this.props.id+'ModifiedEditModal'} className="modal fade" tabIndex={-1} role="dialog" aria-labelledby="modifiedEditModalLabel" aria-hidden="true">
                    <div className="modal-dialog modal-md" role="document">
                        <form onSubmit={this.onSubmit} className="modal-content">
                            <div className="modal-body">
                                <h4>Edit Item</h4>
                                <div className="text-center p-4">
                                    {!this.state.picture?null:<img src={process.env.REACT_APP_API_PUBLIC_URL+this.state.picture} height={250} width={250} className="img-fluid" alt={this.state.id} loading="lazy" />}
                                </div>
...
export default DashboardModifiedEditModal;
